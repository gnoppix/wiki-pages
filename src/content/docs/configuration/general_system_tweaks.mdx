...
:::note
Starting from gnoppix-kde-settings 4.3, SDDM uses KWin as it's Wayland compositor by default.
:::
...
In order to revert this change. **Remove** gnoppix-kde-settings:
```sh
sudo pacman -R gnoppix-kde-settings
```
