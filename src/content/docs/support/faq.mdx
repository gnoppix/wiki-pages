...
:::note
The following questions & answers apply to Gnoppix and Arch Based distributions.
:::
...
The XFCE can be considered as our "flagship" desktop environment, therefore it gets the most maintenance. The other Gnoppix's ISO
...
We recommend installing Gnoppix in a VM for testing out different desktop environments and window managers as the live ISO is only
used for installation and recovering a broken install.

Since we do not currently have community maintainers for other Desktop Environments, those images will be reserved exclusively for Gnoppix members until we find someone who contributes to their maintenance.

:::caution
Please be aware that installing Gnoppix on older Apple Macs will not work out of the box.
:::


### When start archinstall / gnoppixinstall nothing seems to happen...?
...
I'll give you an small heads up on what this [script](https://github.com/Gnoppix/Gnoppix-Live-ISO/blob/master/archiso/airootfs/usr/local/bin/calamares-online.sh) is doing on the background:

<Steps>

1. Removes the existing keyring file found in `/etc/pacman.d/gnupg`.
2. Starts installing and updating both `Arch Linux & Gnoppix keyring packages`.
...
```
:::tip
You can achieve the same outcome by using **"Remove db lock"** function in `Gnoppix Hello` in the **Apps/Tweaks** tab.
:::
...
:::tip
You can also clean the pacman cache using Gnoppix Hello in **Apps/Tweaks** > **Clear package cache**
:::
...
- *Browser due to it's nature of leaning towards privacy. Disables WebGL by default and GitHub depends on it for rendering their homepage*
...
- *No. This is because we include a [policies file](https://github.com/Gnoppix/Gnoppix-Browser-Settings/blob/master/distribution/policies.json) on which we disable features and add modifications to the browser. Example: `additional search engines.`*
